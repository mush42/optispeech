...
@jit(nopython=True)
def _sum_by_duration(ds, xs, text_lengths, feats_lengths):
    B = ds.shape[0]
    xs_sum = np.zeros_like(ds)
    ds = ds.astype(np.int32)
    for b in range(B):
        t_text = text_lengths[b]
        t_feats = feats_lengths[b]
        ...
        for n, (start, end) in enumerate(zip(d_cumsum[:-1], d_cumsum[1:])):
            if len(x[start:end]) != 0:
                xs_sum[b, n] = x[start:end].sum()
            else:
                xs_sum[b, n] = 0
    return xs_sum

def sum_by_duration(ds, xs, text_lengths, feats_lengths):
    """Sum frame-level features into token-level according to durations
    Args:
        ds (Tensor): Batched token duration (B, T_text).
        xs (Tensor): Batched feature sequences to be averaged (B, T_feats).
        text_lengths (Tensor): Text length tensor (B,).
    ...
    args = [ds, xs, text_lengths, feats_lengths]
    args = [arg.detach().float().cpu().numpy() for arg in args]
    xs_sum = _sum_by_duration(*args)
    xs_sum = torch.from_numpy(xs_sum).to(device)
    return xs_sum
